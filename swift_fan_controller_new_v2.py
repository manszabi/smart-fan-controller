...
class ANTPlusInputHandler:
    ...
    def _on_disconnect(self) -> None:
        """ANT+ eszköz kapcsolat megszakadásakor hívódik."""
    ...
    def _init_node(self) -> None:
        ...
        if self.ds.get("power_source", "antplus") == "antplus":
            ...
            meter.on_lost = lambda: setattr(self, "power_connected", False)  # ← új sor
    ...
    def _thread_loop(self) -> None:
        ...
        while self._running.is_set():
            try:
                ...
                self._node.start()  # blokkoló hívás# Blokkoló hívás – itt vár, amíg az ANT+ fut
...
class FanController:
    ...
    def __init__(self, settings_file: str = "settings.json") -> None:
        self.lastdata = 0.0  # ← az __init__-ben
    ...
    async def run(self) -> None:
        ...
        try:
            if self._tasks:
                ...
                # Ha nincs task, csak várakozunk a leállításig
                while self._running:
                    await asyncio.sleep(1)
        except asyncio.CancelledError:
            pass
...
def main() -> None:
    ...
    # asyncio daemon szálban fut
    def run_asyncio() -> None:
        asyncio.set_event_loop(loop)
        try:
            loop.run_until_complete(controller.run())
        except Exception as exc:
            logger.error(f"AsyncioThread hiba: {exc}")
        # NE zárd be itt a loop-ot!
