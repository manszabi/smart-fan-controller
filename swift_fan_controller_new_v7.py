...
class ANTPlusInputHandler:
    ...
    def _init_node(self) -> None:
        ...
        if self.ds.get("hr_source", "antplus") == "antplus" and self.hr_enabled:
            ...
            hr_monitor.on_lost = lambda: setattr(self, "hr_connected", False)
...
class BLEPowerInputHandler:
    ...
    Ha nincs device_name beállítva, automatikus felderítés indul:
    a Cycling Power Service UUID alapján keres megfelelő eszközt.

    Parse: flags (2 bájt LE) → instantaneous power (2 bájt LE, signed int16).

    Attribútumok:
        device_name: A keresett BLE power meter neve (None = auto-discovery).
        is_connected: True, ha a BLE kapcsolat aktív.
    """
    ...
    async def run(self) -> None:
        ...
        Ha nincs device_name, automatikusan keres Cycling Power Service-t
        ...
        if self.device_name:
            ...
            print("📡 BLE Power: nincs eszköznév megadva, automatikus felderítés...")
            logger.info("BLE Power fogadó elindítva (auto-discovery mód)")
    ...
    async def _scan_and_subscribe(self) -> None:
        ...
        Ha device_name üres: auto-discovery a Cycling Power Service UUID alapján.
        """
        if not _BLEAK_AVAILABLE:
            return
        ...
        if self.device_name:
            ...
            if matched is None:
                raise Exception(
                    "BLE Power: nem található Cycling Power Service eszköz – "
                    "újrapróbálkozás..."
                )
            addr = matched.address
            print(
                f"✓ BLE Power auto-csatlakozás: "
                f"{matched.name or '(névtelen)'} ({matched.address})"
            )
...
class BLEHRInputHandler:
    ...
    Ha nincs device_name beállítva, automatikus felderítés indul:
    a Heart Rate Service UUID alapján keres megfelelő eszközt.

    Parse: flags byte bit 0 → 0 = 8-bites HR, 1 = 16-bites HR.

    Attribútumok:
        device_name: A keresett BLE HR eszköz neve (None = auto-discovery).
        is_connected: True, ha a BLE kapcsolat aktív.
    """
    ...
    async def run(self) -> None:
        ...
        Ha nincs device_name, automatikusan keres Heart Rate Service-t
        hirdető eszközt, és folyamatosan próbálkozik, amíg talál egyet.
        """
        if not _BLEAK_AVAILABLE:
        ...
        if self.device_name:
            ...
            print("📡 BLE HR: nincs eszköznév megadva, automatikus felderítés...")
            logger.info("BLE HR fogadó elindítva (auto-discovery mód)")
    ...
    async def _scan_and_subscribe(self) -> None:
        ...
        Ha device_name megadva: név alapján keres.
        Ha device_name üres: auto-discovery a Heart Rate Service UUID alapján.
        """
        if not _BLEAK_AVAILABLE:
            return
        ...
        if self.device_name:
            ...
            if matched is None:
                raise Exception(
                    "BLE HR: nem található Heart Rate Service eszköz – "
                    "újrapróbálkozás..."
                )
            addr = matched.address
            print(
                f"✓ BLE HR auto-csatlakozás: "
                f"{matched.name or '(névtelen)'} ({matched.address})"
            )
        ...
        async with BleakClient(addr) as client:
            ...
            def _handler(sender: Any, data: bytes) -> None:
                try:
                    ...
                    try:
                        self.hr_queue.put_nowait(hr)
                    except asyncio.QueueFull:
                        logger.debug("BLE HR queue teli, adat elvetve")
                except Exception as exc:
